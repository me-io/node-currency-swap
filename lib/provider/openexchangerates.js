 ...
 * @param enterprise A flag to tell if it is in enterprise mode
 * (optional) default false
 */
var OpenExchangeRatesProvider = module.exports = function (appId, enterprise) {
    OpenExchangeRatesProvider.super_.call(this);

    this.appId = appId;
    this.enterprise = enterprise ? enterprise : false;
    ...
    self.fetchContent(url, function (err, content) {
        ...
        try {
            ...
            if (!_.isUndefined(data['error'])) {
                callback(new Error(data['description']), null);
            }
            ...
            callback(null, new Rate(_.toNumber(data.rates[currencyPair.getQuoteCurrency()]), date, Provider));
        }
        catch (err) {
            callback(err, null);
        }
