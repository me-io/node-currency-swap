...
AbstractProvider.prototype.fetchContent = function (url, callback) {
    request('GET', url, function (err, res) {
        if (err) {
            return callback(new Error('Request failed due to connection issue or server does not responded'), null);
        }
...
AbstractProvider.prototype.fetchContentSync = function (url) {
    try{
        ...
        throw new Error('Request failed due to connection issue or server does not responded');
    }
