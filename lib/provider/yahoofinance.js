    ...
    this.parseContent = function (content) {
        try {
            ...
            return {
                ...
                rate: jsonContent.query.results.rate[1]
            };
        }
        catch (err) {
...
YahooFinanceProvider.prototype.fetchRate = function (currencyPair, callback) {
    ...
    self.fetchContent(url, function (err, content) {
        ...
        try {
            ...
            if (data === null || _.isUndefined(data) || _.isUndefined(data.rate) || _.isUndefined(data.created) || _.isUndefined(data.rate.Rate) || data.rate.Rate === 'N/A') {
                return callback(new UnsupportedCurrencyPairException(currencyPair, Provider), null);
            }

            return callback(null, new Rate(_.toNumber(data.rate.Rate), new Date(data.created), Provider));
        }
        catch (err) {
            callback(err, null);
        }
...
YahooFinanceProvider.prototype.fetchRateSync = function (currencyPair) {
    ...
    return new Rate(_.toNumber(data.rate.Rate), new Date(data.created), Provider);
};